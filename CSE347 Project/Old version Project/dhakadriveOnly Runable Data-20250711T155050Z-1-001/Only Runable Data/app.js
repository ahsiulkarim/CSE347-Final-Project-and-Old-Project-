document.addEventListener('DOMContentLoaded', () => {
    ...
    const priceConfirmModal = document.getElementById('price-confirm-modal');
    ...
    // --- ENROLLMENT REQUEST ---
    // Step 1: Show Price Confirmation Modal
    document.querySelectorAll('.request-btn').forEach(button => {
        button.addEventListener('click', (e) => {
            ...
            const existingRequest = enrollments.find(en => en.userId === user.id && en.courseName === courseName && en.status !== 'Payment Rejected' && en.status !== 'Not Available');
            if (existingRequest) {
                alert(`You already have a request for ${courseName}. Check your dashboard for its status.`);
                return;
            }

            courseToRequest = courseName;
            const coursePrice = coursePrices[courseToRequest] || 'N/A';

            if (priceConfirmModal) {
                document.getElementById('modal-price-course-name').textContent = courseToRequest;
                document.getElementById('modal-course-price').textContent = coursePrice.toLocaleString();
                priceConfirmModal.style.display = 'flex';
            }
        });
    });

    // Step 2: On Price Confirmation, show Location Modal
    if (confirmPriceBtn) {
        confirmPriceBtn.addEventListener('click', () => {
            if (priceConfirmModal) priceConfirmModal.style.display = 'none';
    ...
    // Step 3: On Location submit, create the request
    if (requestForm) {
